...
export default function RoomManagement({ initialRooms = [], initialBuildings = [] }) {
  ...
  const [form, setForm] = useState({ room_number: "", floor: 1, area: 25, price: "", buildingId: "" });
  ...
  const openAdd = () => { setEditRoom(null); setForm({ room_number: "", floor: 1, area: 25, price: "", buildingId: buildingFilter !== "all" ? buildingFilter : "" }); setOpenCreate(true); };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", alignItems: { sm: "center" }, gap: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight="bold">Quản Lý Danh Sách Phòng Trọ</Typography>
