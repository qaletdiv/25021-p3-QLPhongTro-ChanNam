...
export default function TenantManagementFilter({ statusFilter, companionStatus, search, dateFrom, dateTo, ttFrom, ttTo, buildings, buildingFilter, onChange }) {
  return (
    <Paper sx={{ p: 2.5, borderRadius: "16px", border: "1px solid #e2e8f0" }}>
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr 1fr" }, gap: 2, mb: 2 }}>
        ...
        {/* Row 1: Companion Status */}
        <Box>
          <Typography sx={{ fontSize: "0.6875rem", fontWeight: 700, color: "#64748b", mb: 0.75 }}>Người Đi Kèm</Typography>
          <Select
            fullWidth size="small" value={companionStatus}
            onChange={(e) => onChange("companionStatus", e.target.value)}
            sx={{ "& .MuiOutlinedInput-root": { fontSize: "0.75rem" }, "& .MuiSelect-select": { fontSize: "0.75rem", py: 1.1 } }}
          >
            <MenuItem value="all">Tất cả</MenuItem>
