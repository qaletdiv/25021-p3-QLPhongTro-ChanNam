...
export default function FilterBar({ filters, activeKey, onFilterChange, counts, total, search, onSearchChange, searchPlaceholder }) {
  return (
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, bgcolor: "#f1f5f9", p: 0.5, borderRadius: "12px", width: { xs: "100%", md: "auto" } }}>
        {filters.map((f) => (
          <Box key={f.key} onClick={() => onFilterChange(f.key)}
            sx={{ px: 1.75, py: 0.9, borderRadius: "8px", fontSize: "0.75rem", fontWeight: 700, cursor: "pointer", bgcolor: activeKey === f.key ? f.activeColor : "transparent", color: activeKey === f.key ? "#fff" : "#475569", boxShadow: activeKey === f.key ? "0 1px 2px rgba(0,0,0,0.05)" : "none", transition: "all 0.15s", whiteSpace: "nowrap" }}
