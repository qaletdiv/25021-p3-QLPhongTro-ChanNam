...
exports.checkConnection = async (landlordId, buildingId) => {
    ...
    if (!botToken) return { ok: false, message: "Chưa cấu hình Telegram Bot Token" };
    const data = await call("getMe", botToken);
    if (data.ok) return { ok: true, message: `Kết nối thành công (bot @${data.result.username})` };
    return { ok: false, message: data.description || "Không thể kết nối Telegram" };
};

exports.sendMessage = async ({ landlordId, buildingId, chatId, text, parseMode }) => {
    const botToken = await getBotToken(landlordId, buildingId);
    if (!botToken) throw new Error("Chưa cấu hình Telegram Bot Token trong Cấu hình");
    if (!chatId) throw new Error("Khách thuê chưa cung cấp Telegram Chat ID");
    const params = { chat_id: String(chatId).trim(), text };
    if (parseMode) params.parse_mode = parseMode;
    const data = await call("sendMessage", botToken, params);
    if (!data.ok) throw new Error(data.description || "Không gửi được tin nhắn Telegram");
    return data;
};
...
exports.sendToLandlord = async ({ landlordId, buildingId, text, url }) => {
    ...
    if (!chatId) throw new Error("Chưa cấu hình Telegram ID cho chủ trọ");
